/**
 * Layered NZA mark - circle + three triangles, paths from
 * public/assets/nza-mark-thick-layered.svg.
 *
 * Inline so fill="currentColor" inherits the host cascade. The mark
 * is used as two stacked layers in the preloader:
 *   - One copy at low opacity = the hollow/outlined state
 *   - One copy at full opacity, clip-pathed from the bottom up over
 *     1.5s = the "glass filling with ink" fill animation
 *
 * Group ids preserved per the design system - if a future treatment
 * wants to animate the four shapes independently it can target them
 * by id.
 */
export function NzaMarkLayered({ className }: { className?: string }) {
  return (
    <svg
      className={className}
